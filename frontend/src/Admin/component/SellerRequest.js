...
function SellerRequest() {
  ...
  const fetchData = async () => {
    const response = await axios.get(
      "http://127.0.0.1:4000/api/admin/getSellerReq",
    ...
    try {
      ...
      if (openDoc.request === "Adding") {
        response = await axios.post(
          "http://127.0.0.1:4000/api/admin/AddItem",
          openDoc,
          { withCredentials: true }
        );
      } else if (openDoc.request === "remove") {
        response = await axios.delete(
          `http://127.0.0.1:4000/api/admin/removeSellerItem/${openDoc._id}`,
          { withCredentials: true }
        );
        console.log(response)
      } else if (openDoc.request === "update") {
        response = await axios.post(
          "http://127.0.0.1:4000/api/admin/UpdateSellerItem",
          openDoc,
          { withCredentials: true }
        );
      }
  ...
  const handleReject = async () => {
    try {
      const response = await axios.put(
        "http://127.0.0.1:4000/api/admin/reject",
        openDoc,
        { withCredentials: true }
      );
