...
function Profile() {
    ...
    const fetchData = async ()=>{
        try {
            const response = await axios.get(`${window.location.origin}/api/user/orders/${user._id}`,{withCredentials:true})
    ...
    const handleUpdate = async (e) => {
        e.preventDefault()
        try {
            const Acknowledge = await axios.patch(`${window.location.origin}/api/user/updateDetails`,
                {
                    _id: user._id,
                    username: username,
